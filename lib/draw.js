...
function loadAllImages() {
  boxPic = loadImage("assets/finished.png");
  wallPic = loadImage("assets/roof.png");
  floorPic = loadImage("assets/ground.png");
  playerPic1 = loadImage("assets/char.png");
  playerPic2 = loadImage("assets/char2.png");
  buttonPic = loadImage("assets/helipad.jpg");
  placedBoxPic = loadImage("assets/box.png");
}
...
function drawButtons(lvl) {
  lvl.buttons.forEach(function (element) {
    image(
      buttonPic,
      element.x * lvl.blockSize,
      element.y * lvl.blockSize,
      lvl.blockSize,
      lvl.blockSize
    );
  });
...
function drawBox(lvl, element) {
  if (element.placed) {
    image(
      placedBoxPic,
      element.px * lvl.blockSize,
      element.py * lvl.blockSize,
      lvl.blockSize,
      lvl.blockSize
    );
  } else {
...
function drawWin() {
  var variable = 255;
  fill(variable, variable, variable, 10);
  rect(0, 0, canvasX, 185, 0);
  textSize(45);
  fill(255 - variable, 255 - variable, 255 - variable);
  text("Victory!", 182, 72);
  textSize(25);
  text("Press enter to continue", 142, 120);
  counter++;
}
